CREATE DATABASE IF NOT EXISTS Buoi15;
...
INSERT INTO Books (BookID, Title, Author, Genre, YearPublished) VALUES
    ...
    (31,'Vũ Trụ Trong Vỏ Hạt Dẻ', 'Stephen Hawking', 'Khoa học', 2001),
    (32,'Lược Sử Thời Gian', 'Stephen Hawking', 'Khoa học', 1988),
    (33,'Tiểu Thuyết Số Một', 'Nguyễn Nhật Ánh', 'Tiểu thuyết', 2012),
    (34,'Tiểu Thuyết Số Hai', 'Nguyễn Nhật Ánh', 'Tiểu thuyết', 2015);
...
INSERT INTO Borrowings (BorrowID, MemberID, BookID, BorrowDate, ReturnDate) VALUES
    ...
    (31,1, 2, '2022-04-12', '2022-04-20'),
    (32,1, 3, '2022-04-22', NULL),
    (33,1, 4, '2022-04-24', NULL),
    (34,1, 5, '2022-04-26', NULL),
    (35,1, 6, '2022-04-28', NULL),
    (36,2, 31, '2022-05-01', '2022-05-05'),
    (37,2, 32, '2022-05-06', NULL),
    (38,3, 29, '2022-05-07', NULL),
    (39,4, 30, '2022-05-08', NULL),
    (40,2, 31, '2022-05-01', '2022-05-05'),
    (41,2, 32, '2022-05-06', NULL);
...
SELECT
    Books.Genre AS TheLoai,
    COUNT(Books.BookID) AS SoLuongSach
FROM Books
GROUP BY Books.Genre;

#5. Liệt kê tên các thành viên và số lần họ mượn sách
SELECT Name AS Ten, COUNT(Borrwings.BorrowID) AS SoLanMuon
FROM Members Member
         LEFT JOIN Borrowings Borrwings ON Member.MemberID = Borrwings.MemberID
...
SELECT Members.Name AS Ten,
       COUNT(Borrowings.BorrowID) AS SoLanMuon
FROM Members
...
SELECT Members.Name AS Ten,
       SUM(DATEDIFF(Borrowings.ReturnDate, Borrowings.BorrowDate)) AS TongNgayGiuSach
FROM Members
         JOIN Borrowings ON Members.MemberID = Borrowings.MemberID
WHERE Borrowings.ReturnDate IS NOT NULL
GROUP BY Members.Name
ORDER BY TongNgayGiuSach DESC
LIMIT 1;

#9.	Liệt kê tên thành viên và số cuốn sách họ đã mượn kèm số sách chưa trả
SELECT Members.Name,
       COUNT(Borrowings.BorrowID) AS TongSoSachDaMuon,
       COUNT(CASE WHEN Borrowings.ReturnDate IS NULL THEN 1 END) AS SoSachChuaTra
FROM Members
...
WHERE Books.Genre = 'Tiểu thuyết'
GROUP BY Members.Name, Books.Genre
HAVING COUNT(*) >= 2;

#11. Tìm thành viên đã mượn sách nhưng chưa trả
SELECT DISTINCT Members.Name
FROM Members
JOIN Borrowings B on Members.MemberID = B.MemberID
WHERE ReturnDate IS NULL;

#12. Liệt kê 3 thể loại sách được mượn nhiều nhất
SELECT Books.Genre, COUNT(*) AS SoLanMuon
FROM Borrowings
         JOIN Books ON Borrowings.BookID = Books.BookID
GROUP BY Books.Genre
